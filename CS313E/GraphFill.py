#  File: GraphFill.py
#  Description: Find and print an adjacency matrix for a graph, and use both depth first and breadth first search algorithms to color pixels in an image. 
...
class ImageGraph:
    ...
    def print_adjacency_matrix(self):
        ...
        # iterate over all nodes and their edges, and mark the corresponding cell in the matrix as 1
        for node in self.nodes:
            for edge in node.edges:
                adjacency_matrix[node.index][edge] = 1
    ...
    def bfs(self, start_index, color):
        ...
        while q.is_empty() != True and count < len(self.nodes):
           curr_node = q.dequeue()
           if curr_node.color == start_color:
               curr_node.visit_and_set_color(color)
           
           for adj_index in curr_node.edges:
              adj_node = self.nodes[adj_index]
              if adj_node.visited == False and adj_node.color == start_color:
                 q.enqueue(adj_node)
    ...
    def dfs(self, start_index, color):
        ...
        start_color = stack.peek().color
        
        while not stack.is_empty():
           curr_node = stack.pop()
           if curr_node.visited == False and curr_node.color == start_color:
              curr_node.visit_and_set_color(color)
