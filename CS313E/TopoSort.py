...
class Graph(object):
    ...
    def toposort(self):
        ...
        for i in self.Vertices:
            if not i.visited:
                self.topoHelp(i, topoList)
        topoList.reverse()
        return topoList

    def topoHelp(self, node, topoList):
        node.visited = tuple
        v = self.Vertices.index(node)
        vx = self.get_adj_vertexes(v)

        for i in vx:
            x = self.Vertices[i]
            if not x.visited:
                self.topoHelp(x,topoList)

        topoList.append(node.label)
